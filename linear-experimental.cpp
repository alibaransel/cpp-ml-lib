...
tuple<vector<double>, double> runGradientDescent(vector<vector<double>> X, vector<double> y, vector<double> w, double b, double alpha, int epoch)
{
    ...
    double prevCost;
    vector<double> prevW = w;
    double prevB = b;

    for (int i = 0; i < epoch; i++)
    {
        cout << setw(12) << alpha * 1000000;
        if (alpha == 0.0)
        {
            cout << "Dynamic Gradient Descent finished by achieving Alpha = 0" << endl;
            break;
        }
        ...
        cout << "Epoc " << setw(8) << i << " Alpha = " << setw(8) << alpha << " w = [";
...
// Epoc 000999 w = [0.285308, 1.018482, 0.006130, 0.048106, 0.019386, ] b = -0.340837 Cost = 4.151353
// Epoc 001130 Alpha = 2.048000 w = [0.285302, 1.018453, 0.006130, 0.048076, 0.019383, ] b = -0.340788 Cost = 4.151351